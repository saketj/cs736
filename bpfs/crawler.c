...
static char zero_block[BPFS_BLOCK_SIZE] __attribute__((aligned(BPFS_BLOCK_SIZE)));

static int crawl_leaf(uint64_t prev_blockno, uint64_t blockoff, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
		crawl_callback callback, void *user, crawl_blockno_callback bcallback,
		uint64_t *new_blockno, uint64_t indir_blockno) {
	...
	if (callback) {
		enum commit child_commit =
				(child_blockno == prev_blockno) ? commit : COMMIT_FREE;
		char *child_block;
		if (is_hole)
		...
		r = callback(blockoff, child_block, off, size, valid, crawl_start,
				child_commit, user, &child_blockno);
...
static int crawl_hole(uint64_t blockoff, uint64_t off, uint64_t size,
		uint64_t valid, uint64_t crawl_start, crawl_callback callback,
		void *user) {
...
static int crawl_indir(uint64_t prev_blockno, uint64_t blockoff, uint64_t off,
		uint64_t size, uint64_t valid, uint64_t crawl_start, enum commit commit,
		unsigned height, uint64_t max_nblocks, crawl_callback callback,
		void *user, crawl_blockno_callback bcallback, uint64_t *new_blockno) {
	uint64_t blockno = prev_blockno;
	...
	case COMMIT_ATOMIC:
		child_commit =
				(firstno == lastno || only_invalid) ? commit : COMMIT_COPY;
		break;
#endif
	case COMMIT_FREE:
	case COMMIT_COPY:
	...
	if (blockno == BPFS_BLOCKNO_INVALID) {
		...
		if (commit == COMMIT_NONE)
			return crawl_hole(blockoff, off, size, valid, crawl_start, callback,
					user);
	...
	for (no = firstno; no <= lastno; no++) {
		...
		if (child_blockno != child_new_blockno || in_hole) {
			...
			if (!(prev_blockno != blockno || (SCSP_OPT_APPEND && only_invalid)
...
void crawl_blocknos(const struct bpfs_tree_root *root, uint64_t off,
		uint64_t size, crawl_blockno_callback callback) {
	...
	if (!tree_root_height(root)) {
		if (!off)
			crawl_leaf(tree_root_addr(root), 0, off, size, valid, off,
					COMMIT_NONE, NULL, NULL, callback, NULL, -1);
	} else {
		crawl_indir(tree_root_addr(root), off / BPFS_BLOCK_SIZE, off, size,
				valid, off, COMMIT_NONE, tree_root_height(root), max_nblocks,
...
static int crawl_tree_ref(struct bpfs_tree_root *root, uint64_t off,
		uint64_t size, enum commit commit, crawl_callback callback, void *user,
	...
	if (commit != COMMIT_NONE) {
		...
		uint64_t int_valid = MIN(root->nbytes,
				BPFS_BLOCK_SIZE * tree_max_nblocks(new_height));
#ifndef NDEBUG
		...
		assert(
				root->nbytes >= new_valid
						|| (root->nbytes < end && end == new_valid));
		assert(root->nbytes <= new_valid || root->nbytes > end);
		assert(root->nbytes != new_valid || root->nbytes >= end);
		assert(new_valid <= BPFS_BLOCK_SIZE * new_max_nblocks);

		if (prev_height < new_height) {
			r = tree_change_height(root, new_height, COMMIT_ATOMIC,
					&new_blockno);
			if (r < 0)
				return r;
	...
	if (!root->ha.height) {
		if (child_size)
			r = crawl_leaf(child_new_blockno, 0, off, child_size, child_valid,
					off, child_commit, callback, user, NULL, &child_new_blockno,
					-1);
		else
			r = 0;
	} else {
		r = crawl_indir(child_new_blockno, off / BPFS_BLOCK_SIZE, off,
				child_size, child_valid, off, child_commit, root->ha.height,
				max_nblocks, callback, user, NULL, &child_new_blockno);
	}
	if (commit != COMMIT_NONE)
		...
		if (commit == COMMIT_NONE) {
			...
			if (r == 0)
				r = crawl_hole((off + child_size) / BPFS_BLOCK_SIZE, child_size,
						size - child_size, root->nbytes, off, callback, user);
...
int crawl_tree(struct bpfs_tree_root *root, uint64_t off, uint64_t size,
...
int crawl_inodes(uint64_t off, uint64_t size, enum commit commit,
	...
	r = crawl_tree(root, off, size, commit, callback, user, &child_blockno);
	if (commit != COMMIT_NONE)
...
static int callback_crawl_inode(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
		void *ccid_void, uint64_t *blockno) {
	struct callback_crawl_inode_data *ccid =
			(struct callback_crawl_inode_data*) ccid_void;
...
int crawl_inode(uint64_t ino, enum commit commit, crawl_callback_inode callback,
		void *user) {
...
static int callback_crawl_data(char *block, unsigned off,
		struct bpfs_inode *inode, enum commit commit, void *ccdd_void,
		uint64_t *blockno) {
	struct callback_crawl_data_data *ccdd =
			(struct callback_crawl_data_data*) ccdd_void;

	return crawl_tree(&inode->root, ccdd->off, ccdd->size, commit,
			ccdd->callback, ccdd->user, blockno);
}

int crawl_data(uint64_t ino, uint64_t off, uint64_t size, enum commit commit,
		crawl_callback callback, void *user) {
...
static void ccd2dd_fill(struct ccd2dd *d, uint64_t ino, uint64_t off,
		uint64_t size, crawl_callback callback, void *user) {
	d->ino = ino;
...
static int callback_crawl_data_2_tree(uint64_t blockoff, char *block,
		...
		enum commit commit, void *ccd2d_void, uint64_t *blockno) {
	struct callback_crawl_data_2_data *ccd2d =
			(struct callback_crawl_data_2_data*) ccd2d_void;
	...
	for (i = 1; i <= 2; i++) {
		if (i & mask) {
			...
			int r = d->callback(blockoff, block, d->off % BPFS_BLOCK_SIZE,
					d->size, valid, crawl_start, c, d->user, blockno);
			if (r < 0) {
...
static int callback_crawl_data_2(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
	...
	if (mask == 3) {
		...
		if (ccd2d->d[0].ino == ccd2d->d[1].ino) {
			...
			return crawl_tree(&inode->root, ccd2d->d[0].off,
					ccd2d->d[1].off - ccd2d->d[0].off + ccd2d->d[1].size,
					commit, callback_crawl_data_2_tree, ccd2d, blockno);
		} else {
			...
			r = crawl_tree(&inode->root, ccd2d->d[0].off, ccd2d->d[0].size,
					COMMIT_COPY, ccd2d->d[0].callback, ccd2d->d[0].user,
					blockno);
			if (r < 0)
				return r;
			block = get_block(*blockno);
			inode_1 = (struct bpfs_inode*) (block + off + size
			...
			r = crawl_tree_ref(&inode_1->root, ccd2d->d[1].off,
					ccd2d->d[1].size, COMMIT_COPY, ccd2d->d[1].callback,
					ccd2d->d[1].user, blockno, false);
		...
		if (mask == 1)
			...
			assert(mask == 2);
			inode = (struct bpfs_inode*) (block + off + size
					- sizeof(struct bpfs_inode));
		}
		assert(commit == COMMIT_COPY);
		return crawl_tree(&inode->root, d->off, d->size, commit, d->callback,
				d->user, blockno);
	}
...
int crawl_data_2(uint64_t ino_0, uint64_t off_0, uint64_t size_0,
		crawl_callback callback_0, void *user_0, uint64_t ino_1, uint64_t off_1,
		uint64_t size_1, crawl_callback callback_1, void *user_1,
		enum commit commit) {
	struct callback_crawl_data_2_data ccd2d;
	...
	return crawl_inodes(ino_start, ino_size, commit, callback_crawl_data_2,
			&ccd2d);
}
