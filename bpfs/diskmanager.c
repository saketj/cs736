...
int readBlocksWithPrefetch(uint64_t blockNumber, char *buf) {
  ...
  if((block = (char *) lru_hash_map_find(prefetch_hash_map, blockNumber)))
  {
    if(block != NULL)
    {
  	...
  	printf("After checking prefetched info inside readblockswithprefetch: %ld\n", blockNumber);
	
	int fd = open(diskManagerFileName, O_RDWR);
    char *tempbuf = (char *)malloc(READ_PREFETCH_BLOCK_COUNT*diskManagerBlockSize);
  	uint64_t offset = (blockNumber) * diskManagerBlockSize;

    uint64_t maxPrefetch = READ_PREFETCH_BLOCK_COUNT;
  	if((currentDiskBlockNumber-1)<blockNumber+READ_PREFETCH_BLOCK_COUNT-1)
  	{
  		maxPrefetch = currentDiskBlockNumber - blockNumber;
  	}
    
    printf("maxprefetch: %ld currentDiskBlockNumber:%ld blockNumber:%ld\n",maxPrefetch, currentDiskBlockNumber, blockNumber);
	assert(pread(fd, tempbuf, maxPrefetch*diskManagerBlockSize, offset)== maxPrefetch*diskManagerBlockSize);
...
uint64_t* writeBlocksInBulk(char **buf, uint64_t numberOfBlocks)
{
  ...
  uint64_t *returnBlockNumbers=(uint64_t *)malloc(sizeof(uint64_t)*numberOfBlocks);
...
int freeBlock(uint64_t blockNumber) {
	...
	char *blockAddress;
	if((blockAddress = (char *) lru_hash_map_find(prefetch_hash_map, blockNumber)))
  {
