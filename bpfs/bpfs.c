...
struct bpfs_super* get_bpram_super(void) {
	assert(BPFS_BLOCKNO_INVALID == 0 && BPFS_BLOCKNO_SUPER == 1);
...
static void move_block_allocations(struct block_allocation *dst,
		struct block_allocation *org) {
...
char* get_block(uint64_t blockno) {
	...
	if (blockno > bpfs_super->nblocks) {
		//assert(0);
		//return NULL;
		//Its possible as MSB is set for disk blocks!
	}
...
uint64_t cow_block(uint64_t old_blockno, unsigned off, unsigned size,
		unsigned valid) {
...
// This code assumes that inodes are contiguous in the inode tree
	assert(!(BPFS_BLOCK_SIZE % sizeof(struct bpfs_inode)));

	return bitmap_init(&inode_alloc.bitmap,
	NBLOCKS_FOR_NBYTES(inode_root->nbytes) * BPFS_INODES_PER_BLOCK);
}
...
static void move_inode_allocations(struct inode_allocation *dst,
		struct inode_allocation *org) {
	bitmap_move(&dst->bitmap, &org->bitmap);
}

static int callback_init_inodes(uint64_t blockoff, char *block, unsigned off,
...
static int callback_get_inode(char *block, unsigned off,
		struct bpfs_inode *inode, enum commit commit, void *pinode_void,
		uint64_t *blockno) {
...
static void mdirent_init_dirent(struct mdirent *md, const struct bpfs_dirent *d,
		uint64_t off) {
...
int tree_change_height(struct bpfs_tree_root *root, unsigned new_height,
	...
	if (new_height > height) {
		...
		while (height_delta-- && new_root_addr != BPFS_BLOCKNO_INVALID) {
			struct bpfs_indir_block *indir =
					(struct bpfs_indir_block*) get_block(new_root_addr);
...
static int callback_read_dir(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
		void *rdd_void, uint64_t *blockno) {
...
static void discover_indir_allocations(struct bpfs_indir_block *indir,
		unsigned height, uint64_t max_nblocks, uint64_t valid) {
	...
	for (no = 0; no <= lastno; no++) {
		if (indir->addr[no] != BPFS_BLOCKNO_INVALID) {
			...
			if (height > 1) {
				struct bpfs_indir_block *child_indir =
						(struct bpfs_indir_block*) get_block(indir->addr[no]);
...
static void tree_complete_shrink(struct bpfs_tree_root *root) {
	if (tree_height(NBLOCKS_FOR_NBYTES(root->nbytes))
			< tree_root_height(root)) {
		// TODO: implement
...
static int read_dir_callback_discover_inodes(uint64_t blockoff, unsigned off,
		const struct bpfs_dirent *dirent, void *mi_void) {
...
static int callback_reset_inodes_nlinks(uint64_t blockoff, char *block,
		unsigned off, unsigned size, unsigned valid, uint64_t crawl_start,
		enum commit commit, void *user, uint64_t *blockno) {
	assert(!(off % sizeof(struct bpfs_inode)));
	assert(commit == COMMIT_FREE || commit == COMMIT_ATOMIC);

	for (; off + sizeof(struct bpfs_inode) <= size; off +=
			sizeof(struct bpfs_inode)) {
		struct bpfs_inode *inode = (struct bpfs_inode*) (block + off);
		inode->nlinks = 0;
	}
	return 0;
...
static void reset_inodes_nlinks(void) {
	xcall(
			crawl_inodes(0, get_inode_root()->nbytes, COMMIT_FREE, callback_reset_inodes_nlinks, NULL));
}
...
static int recover_superblock(void) {
	...
	if (super_2->magic != BPFS_FS_MAGIC) {
		fprintf(stderr,
				"Not a BPFS file system in SP mode (incorrect magic)\n");
		return -1;
	}
...
static void detect_allocation_diffs(void) {
	...
	if (memcmp(orig_block_bitmap, block_alloc.bitmap.bitmap,
					block_alloc.bitmap.ntotal / 8)) {
		diff = true;
		print_bitmap_differences("block", orig_block_bitmap,
				block_alloc.bitmap.bitmap, block_alloc.bitmap.ntotal);
	}
	...
	if (memcmp(orig_inode_bitmap, inode_alloc.bitmap.bitmap,
					inode_alloc.bitmap.ntotal / 8)) {
		diff = true;
		print_bitmap_differences("inodes", orig_inode_bitmap,
				inode_alloc.bitmap.bitmap, inode_alloc.bitmap.ntotal);
	}
...
static int callback_load_directory(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
...
static int callback_get_dirent(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
...
static struct bpfs_dirent* get_dirent(uint64_t parent_ino, uint64_t dirent_off) {
	...
	assert(
			dirent_off + BPFS_DIRENT_MIN_LEN <= get_inode(parent_ino)->root.nbytes);

	r = crawl_data(parent_ino, dirent_off, 1, COMMIT_NONE, callback_get_dirent,
			&dirent);
	if (r < 0)
		return NULL;
	return dirent;
...
static int callback_dirent_plug(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
...
// TODO: set file_type here
	if (!dirent->rec_len) {
		if (off + min_hole_size + BPFS_DIRENT_MIN_LEN <= BPFS_BLOCK_SIZE) {
			...
			struct bpfs_dirent *next_dirent = (struct bpfs_dirent*) (block
					+ next_off);
			int r;
...
static int callback_dirent_append(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
		void *si_void, uint64_t *blockno) {
	struct sd_ino *si = (struct sd_ino*) si_void;
	struct str_dirent *sd = si->sd;
...
static int alloc_dirent(uint64_t parent_ino, struct str_dirent *sd) {
	...
	if (off != DCACHE_FREE_NONE) {
		...
		BPFS_BLOCK_SIZE - (off % BPFS_BLOCK_SIZE), COMMIT_ATOMIC,
				callback_dirent_plug, &si);
		if (r < 0) {
		...
		r = crawl_data(parent_ino, BPFS_EOF, BPFS_BLOCK_SIZE, COMMIT_ATOMIC,
				callback_dirent_append, &si);
		if (r < 0)
			return r;
	}
...
static int callback_set_dirent_ino(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
...
static int callback_clear_dirent_ino(uint64_t blockoff, char *block,
		unsigned off, unsigned size, unsigned valid, uint64_t crawl_start,
		enum commit commit, void *ino_void, uint64_t *blockno) {
	uint64_t *ino = (uint64_t*) ino_void;
	struct bpfs_dirent *dirent;
...
static int callback_addrem_dirent(char *block, unsigned off,
		struct bpfs_inode *inode, enum commit commit, void *cadd_void,
		uint64_t *blockno) {
	struct callback_addrem_dirent_data *cadd =
			(struct callback_addrem_dirent_data*) cadd_void;
...
// placed after crawl_tree() and the cow also guarded by same block?
	...
	dirent_callback =
			cadd->add ? callback_set_dirent_ino : callback_clear_dirent_ino;
	r = crawl_tree(&inode->root, cadd->dirent_off, 1, commit, dirent_callback,
			&cadd->ino, &new_blockno);
	if (r < 0)
		return r;
#if COMMIT_MODE == MODE_BPFS
...
static int callback_init_inode(char *block, unsigned off,
		struct bpfs_inode *inode, enum commit commit, void *ciid_void,
		uint64_t *blockno) {
	struct callback_init_inode_data *ciid =
			(struct callback_init_inode_data*) ciid_void;
	uint64_t new_blockno = *blockno;
...
static int callback_set_cmtime(char *block, unsigned off,
		struct bpfs_inode *inode, enum commit commit, void *new_time_void,
		uint64_t *blockno) {
...
static int create_file(fuse_req_t req, fuse_ino_t parent_ino, const char *name,
		mode_t mode, const char *link, struct bpfs_dirent **pdirent) {
	uint64_t ino;
	size_t name_len = strlen(name) + 1;
	struct str_dirent sd = { { name, name_len }, BPFS_EOF, NULL };
	struct callback_init_inode_data ciid = { mode, fuse_req_ctx(req) };
	struct callback_addrem_dirent_data cadd = { true, 0, BPFS_INO_INVALID,
			S_ISDIR(mode) };
	struct bpfs_inode *inode;
	struct mdirent mdirent;
	int r;
...
// TODO: should we init inodes in alloc and below?
	...
	r = crawl_inode(parent_ino, COMMIT_ATOMIC, callback_set_cmtime,
			&get_inode(ino)->mtime);
	if (r < 0)
		return r;
...
static void fuse_init(void *userdata, struct fuse_conn_info *conn) {
	const char *mode;
	assert(FUSE_ROOT_ID == BPFS_INO_ROOT);
	Dprintf("%s()\n", __FUNCTION__);
	switch (COMMIT_MODE) {
	case MODE_SP:
		mode = "SP";
...
static void fuse_statfs(fuse_req_t req, fuse_ino_t ino) {
	...
	assert(sizeof(stv.f_blocks) >= sizeof(bpfs_super->nblocks));
...
static void fill_fuse_entry(const struct bpfs_dirent *dirent,
...
static void mfill_fuse_entry(const struct mdirent *mdirent,
		struct fuse_entry_param *e) {
	memset(e, 0, sizeof(e));
	e->ino = mdirent->ino;
...
static int truncate_block_zero_leaf(uint64_t prev_blockno, uint64_t begin,
		uint64_t end, uint64_t valid, uint64_t *new_blockno) {
...
static int truncate_block_zero_indir(uint64_t prev_blockno, uint64_t begin,
		uint64_t end, uint64_t valid, unsigned height, uint64_t max_nblocks,
		uint64_t *new_blockno) {
	uint64_t blockno = prev_blockno;
	struct bpfs_indir_block *indir = (struct bpfs_indir_block*) get_block(
			blockno);
	...
	if (begin_aligned)
		...
		if (height > 1)
			r = truncate_block_zero_indir(child_blockno, child_begin, child_end,
					child_valid, height - 1, child_max_nblocks, &child_blockno);
		else
			r = truncate_block_zero_leaf(child_blockno, child_begin, child_end,
					child_valid, &child_blockno);
...
int truncate_block_zero(struct bpfs_tree_root *root, uint64_t begin,
		uint64_t end, uint64_t valid, uint64_t *blockno) {
	uint64_t new_blockno = *blockno;
#if COMMIT_MODE != MODE_BPFS
	unsigned root_off = block_offset(root);
#endif
	...
	if (!begin)
		...
		if (!tree_root_height(root))
			...
			r = truncate_block_zero_indir(child_blockno, begin, end, valid,
					tree_root_height(root), max_nblocks, &child_blockno);
...
static int callback_setattr(char *block, unsigned off, struct bpfs_inode *inode,
		enum commit commit, void *csd_void, uint64_t *blockno) {
	...
	if (!(commit == COMMIT_FREE
			...
			|| (COMMIT_MODE == MODE_BPFS && commit == COMMIT_ATOMIC
	...
	if ((to_set & FUSE_SET_ATTR_UID) && (to_set & FUSE_SET_ATTR_GID)) {
		...
		assert(!(offsetof(struct bpfs_inode, uid) % 8));
		assert(
				offsetof(struct bpfs_inode, uid) + sizeof(inode->uid) == offsetof(struct bpfs_inode, gid));
		assert(sizeof(inode->uid) + sizeof(inode->gid) == 8);
	...
	if (to_set & FUSE_SET_ATTR_SIZE && attr->st_size != inode->root.nbytes) {
		...
		if (attr->st_size < inode->root.nbytes) {
			if (NBLOCKS_FOR_NBYTES(
					attr->st_size) < NBLOCKS_FOR_NBYTES(inode->root.nbytes)) {
...
static void fuse_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr,
	...
	static const int supported = FUSE_SET_ATTR_MODE | FUSE_SET_ATTR_UID
...
static int callback_change_nlinks(char *block, unsigned off,
		struct bpfs_inode *inode, enum commit commit, void *delta_void,
		uint64_t *blockno) {
...
static int do_unlink(uint64_t parent_ino, const struct mdirent *md) {
	...
	struct callback_addrem_dirent_data cadd = { false, md->off,
	BPFS_INO_INVALID, BPFS_S_ISDIR(get_inode(md->ino)->mode) };
	int r;
...
static void fuse_unlink(fuse_req_t req, fuse_ino_t parent_ino, const char *name) {
	const struct mdirent *mdirent;
	int r;
...
#ifndef NDEBUG
static int callback_empty_dir(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
...
#ifndef NDEBUG
	r = crawl_data(mdirent->ino, 0, BPFS_EOF, COMMIT_NONE, callback_empty_dir,
			&parent_ino);
	xassert(r >= 0);
	if (r == 1) {
		bpfs_abort();
...
static void fuse_rename(fuse_req_t req, fuse_ino_t src_parent_ino,
		const char *src_name, fuse_ino_t dst_parent_ino, const char *dst_name) {
	...
	Dprintf(
			"%s(src_parent_ino = %lu, src_name = '%s'," " dst_parent_ino = %lu, dst_name = '%s')\n",
...
// TODO: enable check for MODE_SCSP (need bpram_blockno() support):
	...
	r = crawl_data_2(dst_parent_ino, dst_off, 1, callback_set_dirent_ino,
			(void*) &src_md->ino, src_parent_ino, src_md->off, 1,
			callback_set_dirent_ino, &invalid_ino, COMMIT_ATOMIC);
	if (r < 0)
		goto abort;
...
static int read_dir_readdir(uint64_t blockoff, unsigned off,
	...
	fuse_dirent_size = fuse_add_direntry(params->req, NULL, 0, dirent->name,
	NULL, 0);
	...
	fuse_add_direntry(params->req, params->buf + oldsize,
			...
			DIRENT_FIRST_PERSISTENT_OFFSET + blockoff * BPFS_BLOCK_SIZE + off);
...
static void fuse_readdir(fuse_req_t req, fuse_ino_t ino, size_t max_size,
	...
	while (off < DIRENT_FIRST_PERSISTENT_OFFSET) {
		...
		fuse_add_direntry(req, params.buf + oldsize,
				params.total_size - oldsize, name[name_i], &stbuf, off);
	}
...
static void fuse_create(fuse_req_t req, fuse_ino_t parent_ino, const char *name,
...
static int callback_write(uint64_t blockoff, char *block, unsigned off,
		unsigned size, unsigned valid, uint64_t crawl_start, enum commit commit,
...
void random_fsck(int signo) {
	...
	assert(RFSCK_MAX_INTERVAL <= RAND_MAX);
...
static void init_persistent_bpram(const char *filename) {
	...
	bpram = mmap(NULL, bpram_size, PROT_READ | PROT_WRITE, MAP_SHARED, bpram_fd,
			0);
	xassert(bpram);
...
void inform_pin_of_bpram(const char *bpram_addr, size_t size) __attribute__((noinline));
...
// Initialize and start the anti_cache_manager thread
	...
	if (bpfs_super->version != BPFS_STRUCT_VERSION) {
		fprintf(stderr,
				"File system formatted as v%u, but software is for v%u\n",
...
//  Destroy the anti_cache_manager thread
	if (anti_cache_manager_destroy() != ANTI_CACHE_DESTROY_SUCCESS) {
		fprintf(stderr,
				"Failed to destroy the Anti-Cache Manager thread safely.\n");
		return -1;
	}
