...
void run_bpfs_benchmark(string file_name, int file_size, int anti_cache_size, int hotness_factor) {
  ...
  long disk_offset = 10000;
  long cache_offset = 0;
  printf("disk init offset %ld\n", disk_offset);
  ...
  for (int i = 0; i < blocks_in_cache; ++i) {
    ...
    long offset = cache_offset * BLOCK_READ_SIZE;
  ...
  *shm = 1;
  ...
  for (int i = 0; i < NUM_TRIALS;) {
    ...
    if (bias < hotness_factor) {
      ...
      offset = cache_offset * BLOCK_READ_SIZE;
      printf("cache offset %ld\n", offset);
      cache_offset += 1;
      ++num_bpfs_reads;
    } else {
      // Read from disk.
      ...
      offset = disk_offset * BLOCK_READ_SIZE;
